...
async function loadConversationLog() {
  ...
  try {
    ...
    conversationLog.splice(0,conversationLog.length,...rows.reverse().map(row=>({
      id:String(row.id),time:row.created_at,phone:normalizePhone(row.phone),callId:String(row.call_id||''),
      user:row.user_text||'',gemini:row.gemini_text||''
    })));
  } catch(e){ console.error('Supabase load error:',e.message); }
}
async function persistConversationEntry(entry) {
  if(!SUPABASE_ENABLED)return;
  try{await supabaseRequest('/rest/v1/conversations',{method:'POST',headers:{Prefer:'return=minimal'},
...
async function addConversationEntry({phone,callId,userText,geminiText}) {
  const entry={id:Date.now()+'-'+conversationLog.length,time:new Date().toISOString(),phone:normalizePhone(phone),
    callId:String(callId||''),user:userText||'',gemini:geminiText||''};
...
function sanitizeForYemot(text){return text?String(text).replace(/[."“”‘’']/g,' ').replace(/[-–—]/g,' ').replace(/\s+/g,' ').trim():'';}
function withTimeout(promise,ms,label){
  let timeoutId; const timeoutPromise=new Promise((_,reject)=>{timeoutId=setTimeout(()=>{const e=new Error(`Timeout after ${ms}ms: ${label}`);e.status=408;reject(e);},ms);});
...
async function answerNormalQuestion(audioBase64){
  const prompt=`${EXCLUSIVE_INSTRUCTION?EXCLUSIVE_INSTRUCTION+'\\n\\n':''}זו הקלטה של שאלה מהמתקשר. האזן להקלטה, הבן את הדיבור בעצמך וענה על השאלה.
ענה בשפה שבה המתקשר דיבר. התשובה מיועדת להקראה בטלפון.
אל תצטט תמלול.`;
  return (await generateWithRetry([...audioParts(audioBase64),{text:prompt}])).response.text();
}
async function transcribeForDashboard(audioBase64){
  return sanitizeForYemot((await generateWithRetry([...audioParts(audioBase64),{text:'תמלל את ההקלטה בעברית לצורך תצוגה בלבד. אל תענה על השאלה. החזר רק את התמלול.'}])).response.text());
}
async function answerWithWebSearch(audioBase64){
  const prompt=`${EXCLUSIVE_INSTRUCTION?EXCLUSIVE_INSTRUCTION+'\\n':''}המתקשר ביקש במפורש חיפוש באינטרנט. חפש מידע עדכני ורלוונטי באמצעות Google Search, ואז ענה בעברית על השאלה על סמך המידע שמצאת. אל תציג כתובות אינטרנט.`;
  return (await generateWithRetry([...audioParts(audioBase64),{text:prompt}],true)).response.text();
}
async function buildOpeningForCaller(phone){
  ...
  try{return sanitizeForYemot((await generateWithRetry([{text:`${EXCLUSIVE_INSTRUCTION?EXCLUSIVE_INSTRUCTION+'\\n':''}אתה בתחילת שיחה חדשה עם מתקשר שכבר דיבר איתך בעבר.
הנה קטעים מהשיחות הקודמות:
${history}
צור פתיח קצר בעברית שמזכיר בקצרה את הנושא האחרון, מאפשר להמשיך משם, ושואל על מה המתקשר רוצה לדבר עכשיו. אל תמציא פרטים.`}])).response.text())||'שלום שוב שמח לשמוע ממך על מה תרצה לדבר עכשיו';}
...
async function callHandler(call){
  ...
  while(true){
    ...
    let audioBuffer;
    try{audioBuffer=(await withTimeout(yemotApi.download_file('ivr2:'+recordPath),REQUEST_TIMEOUT_MS,'yemotApi.download_file')).data;}
...
async function configureYemotStructure(){
  ...
  async function updateExtension(path,params){
    const qs=new URLSearchParams({token:apiKey,path,...params});const r=await fetch(`${base}/UpdateExtension?${qs}`);const text=await r.text();
    if(!r.ok)throw new Error(`UpdateExtension HTTP ${r.status}: ${text}`);
    let data;try{data=JSON.parse(text)}catch{data={raw:text}};if(data.responseStatus&&data.responseStatus!=='OK')throw new Error(`UpdateExtension failed: ${text}`);return data;
  }
  ...
  for(const item of voiceMap){const[extension,voice]=item.split(':');if(!extension||!voice)continue;await updateExtension(`ivr2:/2/${extension}`,{type:'add_id_to_list',add_id_to_list_location_list:'/ivr',add_id_to_list_key:'voice',add_id_to_list_value:voice,add_id_to_list_value_change:'yes',add_id_to_list_end_goto:'/1',add_id_to_list_error_end_goto:'/2'});}
}
